...
用于破缓存。当前版本号 `ASSET_V = 20`。

> v20：UI 全新改版为**深色“教师智能教学工作台”**（左侧 7 模块，模块1=人教资源库：教材标签页+章→节目录树+文件卡片+类型筛选+视图切换+上传专区；模块2~7 为建设中占位）。数据底座（`course-data.js`+`resources.json`）与上传/编辑/删除/助手/AI 功能保持不变。
