...
- **✨ 智能简介**：上传/编辑表单里有「✨ 智能简介」按钮，自动写一句资源简介。默认优先用免费大模型，**接口不可用时自动用本地规则**生成
  一句“资源名+章节+类型”的简介，不会失败、不会留空。AI 接口（地址/模型/Key）可在「🔑 管理员登录」弹窗里填。
  注意：免费匿名接口（如 Pollinations）稳定性有限、可能偶发限额/超时；想要稳定建议填一个有免费额度的 key 接口
  （例：硅基流动 `https://api.siliconflow.cn/v1/chat/completions`、模型 `Qwen/Qwen2.5-7B-Instruct`、填你的免费 key）。
