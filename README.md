...
- **上传资源（仅管理员可见）**：右上角**只有你有「上传资源」按钮**，其它访客看不到（他们没有本机令牌）。
  上传即发布到线上：文件写入仓库 `pages/`，并自动登记到 `data/resources.js`，push 到 `main` 后
  GitHub Pages 自动重建，**所有访客都能看到**。
- **支持多种文件类型**：除了 HTML，还支持 **PDF / Word(.docx) / PPT(.pptx) / Excel(.xlsx) / 图片 / 视频 / 压缩包** 等，
  作为文件上传后由浏览器直接打开或下载。
...
- **最简单（推荐，无需本地 Git）**：直接**在网站上“上传资源”**。它会自动把文件写入仓库并登记，
  所有访客可见。**加资源不用碰本地文件、不用记 git 命令。**
- **本地改代码/章节**（如改 `course-data.js`、`resources.js`、样式等）：改完后在 `physics-lib` 里一键发布：
