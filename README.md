...
- **✨ 智能简介**：上传/编辑表单里有「✨ 智能简介」按钮，自动写一句资源简介。填了 AI key 时用大模型生成；否则用本地规则生成一句
  “资源名+章节+类型”的简介，**不会失败、不会留空**。AI 默认用**硅基流动**（国内可达、浏览器可直连、注册送免费额度）。
  启用：在「🔑 管理员登录」弹窗点 `cloud.siliconflow.cn` 注册领额度 → 生成密钥 → 把 key 粘到「AI Key」，模型默认 `Qwen/Qwen2.5-7B-Instruct`。
