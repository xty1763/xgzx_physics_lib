...
## 已实现功能
- **深色“教师智能教学工作台”UI**（v20）：左侧 7 个模块导航，模块1=人教资源库（教材标签页 + 章→节精准目录树 + 文件卡片/列表 + 概览卡 + 类型筛选 + 视图切换 + 上传专区），模块2~7 为“建设中”占位。
  `index.html`/`styles.css`/`app.js` 为本轮重做版本（基于 `物理界面1.html` 设计稿，去掉了不可靠的 chart.js CDN 与外部依赖；保留数据底座 `course-data.js`+`resources.json`）。
- 教材→章→节 目录树（默认展开当前教材，点开选中，再点收起）
- 顶部搜索（标题/描述/标签/教材/章节/类型模糊）+ 目录树内搜索
- 类型筛选：练习/试卷/课件/教案/仿真资源（试卷不挂小节）
- **上传资源**（仅管理员可见）：选文件→自动识别填表（✨自动识别填表，读文件内容+扩展名→识别章节/类型/标签）→保存
- **编辑 / 删除**资源（仅管理员可见）
- 支持 html/pdf/word/ppt/excel/图片/视频/zip，文件按**资源标题**命名
- 右下角 **🤖 资源助手**：自然语言检索资源（解析 教材/章节/类型/关键词；可选接大模型）
- **✨ 智能简介**：自动生成一句话简介（可选大模型，失败用本地规则兜底）

## 关键常量（app.js 顶部）
- `WORKER_URL = "https://physics-lib.xingang-physics.workers.dev"` （**目前指向 Cloudflare Worker，国内连不通，最终应改为腾讯云云函数的 HTTP 触发地址**）
- `ASSET_V = 20`（静态资源版本号，改 app.js/index.html 后需 +1）
...
- **接入可选大模型（AI）**：资源助手（🤖）和「✨ 智能简介」会尽力调用**免费、浏览器可直连、OpenAI 兼容**的接口
  （默认智谱 `https://open.bigmodel.cn/api/paas/v4/chat/completions`，模型 `glm-4-flash` 免费）。失败/超时**自动回退**到本地规则，
  不影响使用。接口地址/模型/Key 在「🔑 管理员登录」弹窗中配置并存入 localStorage（`ai_conf`）。注意：这是**无后端**方案，
  key 只存在站长本机浏览器，仅站长本机能用 AI；其它设备/访客走本地规则兜底。
