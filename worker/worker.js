 ...
 *
 * 校验“授权用户”登录，并在服务端持有 GitHub 令牌，执行 上传/编辑/删除。
 * 前端（静态 GitHub Pages）通过本 Worker 完成这些写操作，令牌不暴露给浏览器。
 *
 * Cloudflare 配置（Worker 的 Secrets / Variables，通过 env 传入）：
 *   Secret: GITHUB_TOKEN、AUTH_SECRET
 *   Var:    GITHUB_OWNER、GITHUB_REPO、GITHUB_BRANCH、USERS_JSON
 */

export default {
  async fetch(request, env) {
    const OWNER = env.GITHUB_OWNER;
    const REPO = env.GITHUB_REPO || "xgzx_physics_lib";
    ...
    const b64decode = (b) => decodeURIComponent(escape(atob(String(b).replace(/\n/g, ""))));
    ...
    async function gh(path, opts = {}) {
      const headers = Object.assign({ Authorization: "Bearer " + TOKEN, "User-Agent": "physics-lib-worker" }, opts.headers || {});
